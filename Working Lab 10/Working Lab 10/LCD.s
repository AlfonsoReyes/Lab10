...
Busy1
	...
	;LSR R1,#3				;isolate bit 4 of SSIO_SR_R, move to bit 1
	...
	BEQ Busy1				;loop if 1 (busy)
	
	;clear D/C = PA6 to zero
	LDR R2,=GPIO_PORTA_DATA_R
	LDR R1,[R2]
	AND R1,#0xFFFFFFBF;	clear PA6 (bit 7) to 0
	STR R1,[R2]
	
	;4)write command SSI0_DR_R
	LDR R2,=SSI0_DR_R	
    LDR R1,[R2]
	STRB R0,[R2]
...
Busy2
	...
	LDR R4,=SSI_SR_BSY
	AND R1,R1,R4			;isolate bit 4 of SSIO_SR_R
	CMP R1,R4
	BEQ Busy2				;loop if 1 (busy)
...
TNF_loop
	...
	LDR R4,=SSI_SR_TNF
	AND R1,R1,R4		;isolate bit 1
	CMP R1,R4
	BNE TNF_loop		;loop if bit 1 is 0 (low)
